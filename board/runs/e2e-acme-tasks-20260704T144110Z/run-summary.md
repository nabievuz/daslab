---
run_id: e2e-acme-tasks-20260704T144110Z
pack: acme-tasks
...
# Run summary — e2e-acme-tasks-20260704T144110Z

Spec->build end-to-end for PROJECT-OS pack `acme-tasks` (`evals/e2e/sample-pack`). One pack driven sample-pack -> compiled stage-gated tickets -> all six AADL gates -> this committed summary.

## Outcome

- Result: **PASS**
- Goals: 4 (slack-notifications, task-crud, team-workspaces, user-auth)
- Compiled tickets: 28 (zero hand-written)
...
1. `gateway_compile.run_pipeline` — compiled the copied pack into 28 self-contained stage-gated story tickets (zero hand-written; every board file is compiler output).
...
- [x] board_lint on the delivered board: 28 tickets, 0 violation(s)
...
- [x] run workspace created and delivered board staged as the LOCAL artifact (e2e-acme-tasks-20260704T144110Z/workspace/delivered-board — scratch, gitignored)
...
```json
{
  "run_id": "e2e-acme-tasks-20260704T144110Z",
  "pack": "acme-tasks",
  "pack_dir": "evals/e2e/sample-pack",
  ...
  "gate_walk": {
    ...
    "negative_probe": {
      ...
      "forced_stage": 2,
      "sample_violation": "DAS-1003: Stage-2 (Design) ticket is 'done' but GATE-1 (Planning) for goal 'user-auth' is open \u2014 a stage may not advance past an open predecessor gate (AADL \u00a70)",
  ...
  "d5_health_check": {
    ...
    "workspace_path": "board/runs/e2e-acme-tasks-20260704T144110Z/workspace",
    "local_artifact": "board/runs/e2e-acme-tasks-20260704T144110Z/workspace/delivered-board",
    "probe": {
      "command": [
        "python3",
        "scripts/gateway_compile.py",
        "<ephemeral-scratch>/acme-tasks",
        "--gate-walk"
      ],
      "ephemeral": true,
    ...
    "checklist": [
      {
        "ok": true,
        "label": "board_lint on the delivered board: 28 tickets, 0 violation(s)"
      },
      {
        "ok": true,
      ...
      {
        "ok": true,
        "label": "run workspace created and delivered board staged as the LOCAL artifact (e2e-acme-tasks-20260704T144110Z/workspace/delivered-board \u2014 scratch, gitignored)"
      },
      {
        "ok": true,
