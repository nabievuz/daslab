# Product — Runtime Instructions (DasLab)

> **Precedence level 5** (see [`../AGENTS.md`](../AGENTS.md) §2). This file is the
> runtime protocol for Product agents; the dept charter ([`CLAUDE.md`](CLAUDE.md))
...
- **Never review your own work.** When ready, set `status: in_review` and assign
  your reviewer per [`../board/ROUTING.md`](../board/ROUTING.md) (Product manager: CPO).
...
- Product artifacts (specs, roadmap, analytics reviews) live under `product/` per the
  charter — reference them by relative path.

## 4 — Skills available

Domain skills (spec authoring, roadmap planning, analytics) are dispatched per role
